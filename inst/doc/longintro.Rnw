...
the methods found in the {\causalTree} routines, which implement many of the
...
for the {\splus} routines of Clark and Pregibon~\cite{Clark92}
 a different acronym
--- Recursive PARTitioning or causalTree --- was chosen.
It is somewhat humorous that this label ``causalTree'' has now become more
common than the original and more descriptive ``cart'',
a testament to the
influence of freely available software.

The {\causalTree} programs build classification or regression models of a very
...
these are computationally very expensive.  Instead {\causalTree} uses one of
...
for $f$.
Suctrisingly, twoing can be calculated almost as efficiently as the
...
of classes.
Twoing is not part of {\causalTree}.

\subsection{Incoctorating losses}
...
The {\causalTree} software implements only the altered priors method.

\subsubsection{Generalized Gini index}

The Gini index has the following interesting intectretation.  Suppose
...
For arbitrary loss matrix of dimension $C> 2$,
 {\causalTree} uses the above formula with
$L_i = \sum_ j L(i,j)$.
...
The model is fit by using the {\causalTree} function.  The first
...
versus risk often has an initial shact drop followed by a relatively flat
...
estimate the missing datum using the other independent variables; {\causalTree}
...
the {\causalTree} routines impose one more constraint during the construction
...
Because the printout is long, the \Co{file} option of \Co{summary.causalTree}
is often useful.
...
The central fitting function is {\causalTree}, whose main arguments are
\begin{itemize}
...
for causalTree is \Co{na.causalTree}, this default is not overridden by the
...
most useful feature of {\causalTree} models.
\item \Co{control}: a list of control parameters, usually the result of
the \Co{causalTree.control} function.  The list must contain
\begin{itemize}
...
direct intectretation: if any split does not increase the overall
...
1990 Consumer Reports summary on 117 cars. For our puctoses, car
...
\begin{figure}[tb]
  ...
  \caption{Displays the causalTree-based model relating automobile
...
large.
For instance, the call \Co{causalTree(Reliability ~ ., data=car90)}
...
\begin{figure}
  ...
  \caption{Displays the causalTree-based models for the
...
\begin{figure}
  ...
  \caption{Both plots were obtained using the function
    \Co{rsq.causalTree(fit3)}. The figure on the left shows that the first split
...
model. The function \Co{rsq.causalTree} plots the jackknifed error versus
...
puctoses both nodes have the same predicted value (No) and the
...
is appealing, but has not yet been implemented in {\causalTree}.



\section{Poisson regression}
\subsection{Definition}
The Poisson splitting method attempts to extend {{\causalTree}} models to
event rate data.
...
arrived at by puctosely over-fitting the data and then pruning back the
...
This estimate is scale invariant, has a simple intectretation, and
...
In this call, the \Co{causalTree.control} options are modified:
...
\begin{figure}
  ...
  \caption{The first figure shows the solder data, fit with the
    ...
    \Co{prune.causalTree} was used to produce the smaller tree.}
  \label{poissonfig1}
\end{figure}
...
\Co{cp = 0.10} in the original call to {\causalTree}.
...
landed. To create new variables based on the causalTree groupings,
...
\begin{figure}
  ...
  \caption{Survival plot based on snipped causalTree object.  The
...
plotting an causalTree object.  For simplicity, the same model (data from
Example 1) will be used throughout.
You have doubtless already noticed the use of \texttt{par(mar =)}
in the examples.
The plot function for causalTree uses the general \texttt{plot} function
to set up the plot region.  By default, this leaves space for
axes, legends or titles on the bottom, left, and top.
Room for axes is not needed in general for causalTree plots, and
...
\Co{post.causalTree}, whose default action of creating a .ps file
...
at a ``good'' angle.
A separate package \texttt{causalTree.plot} carries these ideas much further.


\section{Other functions}
A more general approach to cross-validation can be gained using the
\Co{xpred.causalTree} function.
Given an causalTree fit, a vector of $k$ complexity parameters,
...
$$
 15* \{(259/315)(39120/67081)  - [(174/315)(240/841) + (85/315)(60/289)]\}
         \approx 3.9876
$$
As is true in general with the causalTree routines, variables with missing
