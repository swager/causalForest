...
The causalTree code was written in a modular fashion with the idea that the C
...
\texttt{func\_table.h} file expanded, the R function \texttt{causalTree} updated to
recognize the new
option (it calls the C routine with the row number of func\_table),
and finally an initialization function written.
See the \texttt{causalTree.class}, \texttt{causalTree.anova}, and \texttt{causalTree.exp}
...
nevertheless this allows an easy way to extend \texttt{causalTree}, and to
test out new
...
\begin{description}
  \item[y] the response value as found in the formula.
    Note that causalTree will normally
...
\begin{description}
  ...
  \item[summary] optional: a function which will produce a 1--3 line summary for the
    node, to be used by \texttt{summary.causalTree}.
...
\begin{itemize}
  ...
  \item use.n = TRUE/FALSE, see the help page for \texttt{text.causalTree}
\end{itemize}
...
\texttt{xpred.causalTree} routine.
...
and an causalTree fit for $Z$ treating $\eta_1$ as an offset.
